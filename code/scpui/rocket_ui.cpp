...
bool transform_to_rocket(vec2d& in) {
	in = {
		in.x - render_offset.x,
		in.y - render_offset.y
	};
	...
	if (evt.type == SDL_KEYUP) {
		...
		if (evt.key.keysym.sym == SDLK_r && ctrl_down && shift_down) {
			...
			if (!debugger_initialized) {
				// First we need to initialize the debugger. We use the current context for that, should work in most cases
...
void setOffset(float x, float y) {
	rendering_interface->setRenderOffset({
											 x,
											 y
										 });
}
