...
CompiledGeometryHandle RocketRenderingInterface::CompileGeometry(Vertex* vertices,
																 int num_vertices,
																 int* indices,
																 int num_indices,
	...
	if (texture == 0) {
		geom->bitmap_handle = -1;
	} else {
		geom->bitmap_handle = get_texture(texture)->handle;
	}
...
void RocketRenderingInterface::RenderCompiledGeometry(CompiledGeometryHandle geometry, const Vector2f& translation) {
	...
	renderGeometry(geom->vertex_buffer, geom->index_buffer, geom->num_elements, geom->bitmap_handle, translation);
}
...
bool RocketRenderingInterface::LoadTexture(TextureHandle& texture_handle,
...
bool RocketRenderingInterface::GenerateTexture(TextureHandle& texture_handle,
...
void RocketRenderingInterface::RenderGeometry(Vertex* vertices,
											  int num_vertices,
											  int* indices,
											  int num_indices,
											  TextureHandle texture,
	...
	if (texture == 0) {
		...
		bitmap = get_texture(texture)->handle;
	}

	renderGeometry(vertex_stream_buffer, index_stream_buffer, num_indices, bitmap, translation);
}

void RocketRenderingInterface::setRenderOffset(const vec2d& render_offset) {
...
void RocketRenderingInterface::renderGeometry(int vertex_buffer,
											  int index_buffer,
											  int num_elements,
											  int bitmap,
